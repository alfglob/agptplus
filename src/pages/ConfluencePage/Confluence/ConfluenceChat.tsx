...
const getSystemPrompt = (studio: string, mode: number[]) => {
  const data = MOCK_CONF_PROJECT_SETUP[studio].groups[mode[0]].groups;
...
export const ConfluenceChatComponent = ({
  ...
  const getNextMode = () => {
    const data = MOCK_CONF_PROJECT_SETUP[studio].groups[confluenceMode[0]].groups;
  ...
  const handleCreatePage = async () => {
    ...
    if (confluenceParents.length === 1 && confluenceMode.length === 3) {
      const groupData = MOCK_CONF_PROJECT_SETUP[studio].groups[confluenceMode[0]].groups![confluenceMode[1]];
  ...
  useEffect(() => {
    if (modeRef.current != null && JSON.stringify(confluenceMode) === JSON.stringify(modeRef.current)) {
      return;
    }
