...
function getExtraName(key: string) {
  if (key === MOCK_CONF_JIRA_PROMPT_DATA.epic.id) {
    return 'Show the KPI for the epic';
  }
  if (key === MOCK_CONF_JIRA_PROMPT_DATA.initiative.id) {
...
export const PromptFormComponent = ({ system, updateSystemMessage, formData, updateField, messages }: any) => {
  ...
  const handleSelect = (ev: any) => {
    ...
    const data = (MOCK_CONF_JIRA_PROMPT_DATA as any)[studio];
    if (!studio) {
      return;
    }
  ...
  // Temp
  useEffect(() => {
    updateSystemMessage(MOCK_CONF_JIRA_PROMPT_DATA['ui-engineering'].prompt);
    updateField({ key: FormDataKeys.CP_SYSTEM, value: MOCK_CONF_JIRA_PROMPT_DATA['ui-engineering'].id });
  }, []);
  ...
  return (
    ...
    >
      ...
      <Box sx={{ gridColumn: 'span 4 / span 4', display: 'flex' }}>
        <Select
          ...
          renderValue={(val) => {
            ...
            return (MOCK_CONF_JIRA_PROMPT_DATA as any)[val]?.display_name;
          }}
          displayEmpty
          size="small"
          value={currentKey ?? ''}
        ...
        >
          {Object.values(MOCK_CONF_JIRA_PROMPT_DATA).map((value) => (
      ...
      {currentKey === MOCK_CONF_JIRA_PROMPT_DATA.epic.id && (
        <EpicKPI show={showExtra} onClose={() => setShowExtra(false)} />
      )}
      {currentKey === MOCK_CONF_JIRA_PROMPT_DATA.initiative.id && (
